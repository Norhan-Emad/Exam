export class Area {
    ...
    displayArea(category) {
        ...
        for (let i = 0; i < category.meals.length; i++) {
            box += `
            <div class="col-md-3 col-sm-6 mb-4 text-white text-center" value="${category.meals[i].strArea}">
            <i class="fa-solid fa-house-laptop fa-4x"></i>
            <h2>${category.meals[i].strArea}</h2>
            </div>
            `;
        }
    ...
    async getFoodArea(word) {
        loading.classList.remove("d-none");
        document.querySelector(".background").classList.remove("d-none");
        const data = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?a=${word}`);
        const response = await data.json();
        loading.classList.add("d-none");
        document.querySelector(".background").classList.add("d-none");
        document.querySelector(".openPart").classList.remove("d-none");
        // console.log(response);
        this.displayFoodArea(response);
    }

    displayFoodArea(category) {
        let box = ``;
        for (let i = 0; i < category.meals.length; i++) {
            box += `
