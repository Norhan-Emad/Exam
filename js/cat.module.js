export class Category {
   ...
   displayCat(category) {
      ...
      for (let i = 0; i < category.categories.length; i++) {
         box += `
         <div class="col-md-3 col-sm-6 mb-4 text-white" value="${category.categories[i].strCategory}">
         <div class="image">
         <img src="${category.categories[i].strCategoryThumb}" alt="image" class="rounded-3 w-100">
            <div class="title"><h3>${category.categories[i].strCategory}</h3>
            </div>
            </div>
            </div>`;
      }
      ...
      document.querySelectorAll("#display .col-md-3").forEach((food) => {
         food.addEventListener("click", () => {
            this.getFoodArea(food.getAttribute("value"));
            console.log(food.getAttribute("value"));
         })
      })
   }

   async getFoodArea(word) {
      loading.classList.remove("d-none");
      document.querySelector(".background").classList.remove("d-none");
      const data = await fetch(`https://www.themealdb.com/api/json/v1/1/filter.php?c=${word}`);
      const response = await data.json();
      loading.classList.add("d-none");
      document.querySelector(".background").classList.add("d-none");
      document.querySelector(".openPart").classList.remove("d-none");
      console.log(response);
      this.displayArea(response);
   }

   displayArea(category) {
      let box = ``;
